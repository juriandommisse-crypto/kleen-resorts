...
/** A1-bereik met correct geciteerde tabbladnaam (spaties/quotes veilig). */
function rangeOf(tab: string, a1: string): string {
...
const PREFERRED_LEADS_TAB = "Leads";

/** Parse het ruwe lead-tabblad (incl. headerrij) naar wekelijkse leads per project. */
function parseLeads(values: string[][]): WeeklyLeads[] | null {
  const hi = values.findIndex(
    (r) =>
      headerIndex(r, "dealid", "deal id", "deal") >= 0 &&
      headerIndex(r, "weekkey", "week key") >= 0 &&
      headerIndex(r, "park") >= 0,
  );
  if (hi < 0) return null;

  const header = values[hi].map(String);
...
export async function fetchWeeklyLeads(): Promise<WeeklyLeads[]> {
  ...
  // Snelste pad: lees het bekende tabblad direct (1 leesverzoek).
  try {
    const direct = parseLeads(await readRange(sheetId, rangeOf(PREFERRED_LEADS_TAB, "A1:Z100000")));
    if (direct) return direct;
  } catch {
    // Tabblad heet mogelijk anders -> val terug op het scannen van headers.
  }
  ...
  for (const [tab, hdr] of headers) {
    if (
      headerIndex(hdr, "dealid", "deal id", "deal") >= 0 &&
      headerIndex(hdr, "weekkey", "week key") >= 0 &&
      headerIndex(hdr, "park") >= 0
    ) {
      const parsed = parseLeads(await readRange(sheetId, rangeOf(tab, "A1:Z100000")));
...
const PREFERRED_SPEND_TAB = "Spend per park";

/** Parse de per-platform spendtabel (headerrij mag ergens in de waarden staan). */
function parseSpend(values: string[][]): WeeklySpend[] | null {
  const hi = values.findIndex(
    (r) => r.some((c) => /spend\s*meta/i.test(c)) && r.some((c) => /maand|month/i.test(c)),
  );
  if (hi < 0) return null;

  const header = values[hi].map((x) => String(x));
...
export async function fetchWeeklySpend(): Promise<WeeklySpend[]> {
  ...
  // Snelste pad: lees het bekende tabblad direct (1 leesverzoek).
  try {
    const direct = parseSpend(await readRange(sheetId, rangeOf(PREFERRED_SPEND_TAB, "A1:Z3000")));
    if (direct && direct.length) return direct;
  } catch {
    // Tabblad heet mogelijk anders -> val terug op scannen.
  }

  const tabs = await listTabs(sheetId);
  for (const tab of tabs) {
    const parsed = parseSpend(await readRange(sheetId, rangeOf(tab, "A1:Z3000")));
